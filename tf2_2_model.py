...
class CycleGAN_2(object):
    ...
    def train(self, args):
        ...
        c_loss_metric = tf.keras.metrics.Mean('c_loss', dtype=tf.float32)
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(batch_idxs):
                ...
                print("real_A {:6.2f}, fake_B {:6.2f}, cycle_A {:6.2f}".format( tf.reduce_sum(real_A).numpy(), tf.reduce_sum(fake_B).numpy(), tf.reduce_sum(cycle_A).numpy()))
                ...
                if np.mod(counter, args.tb_print_freq) == 0:
                    with train_summary_writer.as_default():
                        ...
                        tf.summary.scalar('c_loss', c_loss_metric.result(), step=counter)
